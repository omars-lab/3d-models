import"./modulepreload-polyfill-EeOZK34R.js";/* empty css               */import{n as e,r as t,t as n}from"./note-BIxa1zJp.js";import{A as r,N as i,S as a,T as o,b as s,h as c,m as l,n as u,p as d,t as f,w as p,x as m}from"./dist-C49kJYEf.js";import{i as h,r as g}from"./lego-scripts-H4vlAN16.js";var _=6;function v(e,t){let n=Math.cos(t),r=Math.sin(t);return{x:e.x*n-e.y*r,y:e.x*r+e.y*n}}function y(e){return Math.round(e/8)*8}function b(e,t,n){let r=v(t,n),i={x:y(r.x),y:y(r.y)},a=Math.abs(r.x-i.x),o=Math.abs(r.y-i.y);return{label:e,at:r,snap:i,resX:a,resY:o,worstMm:Math.max(a,o)}}function x(e){let t=o(e.basis),n=e.atTheta===`zero`?0:t.thetaDeg,r=n*Math.PI/180,i=e.studsHalf??3,[a,s]=e.basis;return{name:e.name,report:t,thetaDeg:n,vectors:[b(`a₁`,a,r),b(`a₂`,s,r)],points:S(e.basis,r,e.generations??2,i*8),studsHalf:i,unbuildable:e.unbuildable??!1}}function S([e,t],n,r,i){let a=[];for(let o=-r;o<=r;o++)for(let s=-r;s<=r;s++){if(ee.has(`${o},${s}`))continue;let r=v({x:o*e.x+s*t.x,y:o*e.y+s*t.y},n);te(r,i)&&a.push(r)}return a}var ee=new Set([`0,0`,`1,0`,`0,1`]),te=(e,t)=>Math.abs(e.x)<=t&&Math.abs(e.y)<=t;function C(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`)}function w(e,t=6){let n=e.studsHalf*8,r=2*(n+_)*t,i=e=>(e+n+_)*t,a=e=>(n+_-e)*t,o=e=>Number(e.toFixed(2)).toString(),s=[];for(let t=-e.studsHalf;t<=e.studsHalf;t++){let e=t*8;s.push(`<line class="lat-grid" x1="${o(i(-n))}" y1="${o(a(e))}" x2="${o(i(n))}" y2="${o(a(e))}" />`,`<line class="lat-grid" x1="${o(i(e))}" y1="${o(a(-n))}" x2="${o(i(e))}" y2="${o(a(n))}" />`)}for(let n=-e.studsHalf;n<=e.studsHalf;n++)for(let r=-e.studsHalf;r<=e.studsHalf;r++)s.push(`<circle class="lat-stud" cx="${o(i(n*8))}" cy="${o(a(r*8))}" r="${o(l/2*t)}" />`);for(let n of e.points)s.push(`<circle class="lat-pt" cx="${o(i(n.x))}" cy="${o(a(n.y))}" r="${o(.7*t)}" />`);for(let n of e.vectors)s.push(...ne(n,{X:i,Y:a,n:o,scale:t}));s.push(`<circle class="lat-origin" cx="${o(i(0))}" cy="${o(a(0))}" r="${o(.9*t)}" />`);let c=`${e.name} — grid fit ${e.report.fit===void 0?`n/a`:e.report.fit.toFixed(2)} at ${e.thetaDeg.toFixed(1)} degrees`;return`<svg class="lattice-plan${e.unbuildable?` unbuildable`:``}" viewBox="0 0 ${o(r)} ${o(r)}" role="img" aria-label="${C(c)}" xmlns="http://www.w3.org/2000/svg">${s.join(``)}</svg>`}function ne(e,{X:t,Y:n,n:r,scale:i}){let a=[`<line class="lat-vec" x1="${r(t(0))}" y1="${r(n(0))}" x2="${r(t(e.at.x))}" y2="${r(n(e.at.y))}" />`,`<circle class="lat-tip" cx="${r(t(e.at.x))}" cy="${r(n(e.at.y))}" r="${r(.8*i)}" />`,`<text class="lat-lbl" x="${r(t(e.at.x/2)+6)}" y="${r(n(e.at.y/2)-4)}">${C(e.label)}</text>`],o=e.worstMm<d,s=o?`lat-res ok`:`lat-res`;if(a.push(`<path class="${s}" d="M ${r(t(e.at.x))} ${r(n(e.at.y))} L ${r(t(e.snap.x))} ${r(n(e.at.y))} L ${r(t(e.snap.x))} ${r(n(e.snap.y))}" fill="none" />`),!o){let i=e.resX>=e.resY,o=i?(e.at.x+e.snap.x)/2:e.snap.x,s=i?e.at.y:(e.at.y+e.snap.y)/2;a.push(`<text class="lat-lbl lat-lbl-res" x="${r(t(o))}" y="${r(n(s)-5)}" text-anchor="middle">${e.worstMm.toFixed(2)} mm</text>`)}return a}function re(e,t){return w(x(e),t)}var ie={square:e=>[{x:e,y:0},{x:0,y:e}],hexagonal:e=>[{x:e,y:0},{x:e/2,y:e*Math.sqrt(3)/2}],rhombic72:e=>[{x:e,y:0},{x:e*Math.cos(72*Math.PI/180),y:e*Math.sin(72*Math.PI/180)}],rectangular32:e=>[{x:e,y:0},{x:0,y:1.5*e}]},T={square:{probe:8,best:8},hexagonal:{probe:8,best:17.57},rhombic72:{probe:8,best:6.995},rectangular32:{probe:8,best:16}};function E(e,t,n={}){let r={name:`${e} at ${t} mm`,basis:ie[e](t),studsHalf:n.studsHalf,unbuildable:n.unbuildable};return{model:x(r),svg:re(r)}}function D(e){let t=g(e);if(!t)throw Error(`design note references a preset the Lab does not ship: ${e}`);let n=s(t.source);if(!n.brick3d)throw Error(`${e} compiled to no brick`);return n.brick3d.grid}function O(e){return e.fit===void 0?`n/a`:e.fit.toFixed(2)}function k(e){return e.repeatUnitStuds?.join(` × `)??`n/a`}var A={id:`lattice-basis`,title:`The lattice matrix has a row the grammar cannot build`,eyebrow:`3d-models · docs/lego-lab-design.md · §5.3, §11 Q8`,date:`2026-07-31`,status:`decided`,decision:`<strong>Label the row.</strong> §5.3's table gains an <strong>Authorable</strong>
...
<p class="foot">
  ...
  triangles. Everything else on this page was compiled when you loaded it.
    ...
    plan compiled from <code>patterns/Lego/</code> when you opened this page.`,render(){let t=J(`pin-rail`),r=J(`grid-field-tile`),i=J(`hex-field-tile`),a=J(`star-brick`),o=J(`edge-stud-tile`),s=W(t),c=W(r),l=W(i),d=W(a),f=K(t,5),p=K(r,5),m=K(i,5),h=fe(a,6),g=H(o),_=u.toFixed(0);return`
<section class="panel defect stack">
...
<section class="panel stack">
  ...
  ${e({svg:m,caption:Z(l,`The worst case in the catalogue, and a 6×6 — the same footprint as
      <code>Edge-Stud-Tile</code>, which spans ${W(o).span.diameterMm.toFixed(2)} mm.
      What separates them is ${l.droppedForRelief} rejected candidates against
