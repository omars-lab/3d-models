...
module bottom_lantern_1(arch_height = 40) {
    ...
    translate([0, -arch_height]) difference() {
        union() {
            ...
            translate([0, arch_height + arch_height/3.5]) scale([arch_height/20,arch_height/20,1]) polygon([[-1/2,0], [0,1], [1/2,0]]);
        }
...
module Lantern1(size=40) {
    union() {
        rotate([0, 0, 90]) difference() {
            square([size,size*1.5], center=true);
            translate([0,size*1.45]) circle(size);
            translate([0,-size*1.45]) circle(size);
        }
...
cookie_cutter(10,5, total_height=35) {
    scale([0.65, .75]) Lantern1(500);
}
