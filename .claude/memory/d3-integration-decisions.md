...
- **Q-VOCAB convergence point is `faceKey`, not `joinFaces`** (the K7 the build caught): the shared join key `String(index)` is what all three d3 surfaces hold in common. A's `joinFaces()` *creates* `<path class="face">`; the orb-instrument keeps its OWN `.data().join()` by design — it binds status onto pre-rendered `<path data-face-index>` nodes and asserts enter/exit empty, which a path-creating join cannot. sacred-patterns grew a `faceConstructs()` mapper + `joinFaces()` renderer replacing imperative `<polyline>`, pixel-identical (259 faces, frozen coordinate golden); its `ring` stays undefined (faces come from polygons/stars, never circles — the §9 K10 transfer condition). Canonical terms: `index`/`polygon`/`ring`/`faceKey`/`joinFaces`.
