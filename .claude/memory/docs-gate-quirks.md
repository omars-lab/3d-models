...
- The pointer gate (hook 35) fails a baselined path that EXISTS on disk with "resolves now — delete its entry". The coaster minis (`src/Coasters/<id>-coaster-mini.stl`) are gitignored build output kept in `doc-pointer-baseline.json`, so rendering them for a smoke test makes the gate red: `rm` the rendered minis before `make validate` and keep their baseline entries (2026-09-22, CS-10/CS-11). New baseline entries also need `DOC_POINTERS_BASELINE_MAY_GROW=1`, and hook 45 needs `npm install --prefix tools/bambu` in a fresh worktree.
