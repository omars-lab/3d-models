...
### Housekeeping after the border merges — DONE 2026-09-18

- ~~`git worktree remove ~/Workspace/git/bikar-placeborder`~~ — done; all `bikar-border*` and
  `bikar-placeborder` worktrees are gone. Remaining: `3d-models-constructions` on the merged
  `feat/coaster-border-catalog` (remove when free).
- ~~Update TaskList: #36 completed; #12 drop P3.1/P3.3~~ — the border tasks are done; a fresh
  live TaskList (#1–#10) now carries only the open tail below (P3.1/P3.3 already dropped).
- ~~Memory `feedback` note~~ — done: [[parallel-opus-subagents-in-worktrees]] updated with the
  zero-conflict confirmation and the new skill/hook (Opus subagents, not Fable, in this run).
