...
Phase A (software prep, now) → hand B1–B3 to Omar → #8 bring-up → **Plate 1** (B4, owner-gated) → propagate →
Plates 2–5+. Existing tasks **#8** (bring-up) and **#9** (first dispatch) already track the gated milestones;
Phase A becomes new tracked tasks (register X2D target; dual-nozzle decision; re-verify coupons; ready slice
profile; eyeball MC-4; capture the gap; **produce the Plate 1 bench sheet**; **encode the photograph→compare
→verdict loop in `prototype` + the prints gate**). Phase A ships regardless of when Omar toggles the
touchscreen, so the campaign is never idle waiting on hardware — the bench sheet makes Plate 1 physically
readable the moment it comes off the bed, and the extended skill + gate make "did the print match what we
expected?" a scripted, enforced step instead of a thing a tired operator remembers to do.
