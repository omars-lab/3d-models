...
- **Counts reconcile — and are copied from the thing that prints them, not
  re-derived here.** 17 bets = 7 on the card + 9 on design coupons
  ...
  `CAL-FRM-01` W-P1) + 1 with no coupon (`CAL-STR-01`). 17 records = 12 on the
  ...
  record count does not track the bet count. 29 print-gated items = 29 catalog
