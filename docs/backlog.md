...
| `tile-wall` §10 Q2 — `checker` border parity | **decided 2026-08-03, D-016: both**; **specified 2026-08-03** as [`tile-wall-design.md`](tile-wall-design.md) §8.1, with the offset `FAIL:` and the aggregate warning. Writing it found the trap the decision missed — adjacent tiles traverse the shared edge in opposite senses, so the compare must be in wall coordinates. Still unbuilt in bikar: the code | [`tile-wall-design.md`](tile-wall-design.md) §8.1, [`decisions-log.md`](decisions-log.md) D-016 |
| `tile-wall` §10 Q3 — cropped-edge finish | **decided 2026-08-03, D-017: both finishes, but `frame` is orthogonal to `crop`**; **band width settled 2026-08-03** as a D3 default in [`tile-wall-design.md`](tile-wall-design.md) §3.2 — and *not* as the calibration bet this row predicted. It is `max(tile border, widest cropped fragment on that edge)`, derived per edge; a bet would have been a category error, since a bet is a quantity only a caliper settles and the layout already computes this one | [`tile-wall-design.md`](tile-wall-design.md) §3.2, [`decisions-log.md`](decisions-log.md) D-017 |
