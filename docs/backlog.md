...
**22 <!--count:cal-bets--> registered bets · 21 <!--count:cal-records-->
`Calibrated` records — 21 provisional, 0 measured · 7
...
and `qiyas`. 22 <!--count:cal-bets--> ids are registered in `CAL_BETS` <!--count:partial--> (twelve at the original
...
| | Count | Note |
|---|---|---|
| Registered `CAL-*` bets | 22 <!--count:cal-bets--> | 20 with a coupon, 7 <!--count:cal-bets-no-record--> without a record in bikar (`CAL-OVH-01`, `CAL-STR-01`, the two mural bets `CAL-REG-01`/`CAL-CLB-01`, the two brick-anchor bets `CAL-ANC-01`/`CAL-INW-01`, and the equivalence bet `CAL-EQV-01`) |
| `Calibrated<T>` records | 21 <!--count:cal-records--> | all provisional, all listed in bikar's `.calibration-baseline.json` |
| Bets settled by the machine card (MC-1…MC-8) | 10 <!--count:cal-bets-mc--> | 15 <!--count:cal-mc-records--> of the 21 <!--count:cal-records--> records |
| Bets settled by design-specific coupons | 10 <!--count:cal-bets-design--> | `CAL-RIB-01` (LG-F1), `CAL-STK-01` (LG-S1), `CAL-DET-01` + `CAL-CLP-01` (W-C1), `CAL-REG-01` (LG-P1), `CAL-CLB-01` (LG-P2), `CAL-ANC-01` + `CAL-INW-01` (LG-B2), `CAL-FRM-01` (W-P1), `CAL-GRP-01` (LG-D1) — but only 6 <!--count:cal-design-records--> records, because four of the ten have a coupon and no `Calibrated` record yet |
| Bets with no coupon anywhere | 2 <!--count:cal-bets-no-coupon--> | `CAL-STR-01`, Z-layer strength ratio — registry says it "needs a load rig, which does not exist"; `CAL-EQV-01`, the O2 centreline-coverage floor — not a print quantity, settled by the corpus ladder |
| Entries in the prototype catalog | 32 <!--count:catalog-entries--> | 28 coupons (P1–P8, MC-1…MC-8, W-F1, W-C1, W-P1, LG-F1/F2/S1/R1/D1/B1/B2/P1/P2) + the 4 deliverables C1, C2, W1, W2 catalogued 2026-08-03 (§3.5). Count is the one `make validate-catalog` prints, not a hand tally |
...
22 <!--count:cal-bets--> registered bets and 15 <!--count:cal-mc-records--> of the
21 <!--count:cal-records--> provisional records are settled by an MC-series coupon —
...
`CAL-FRM-01`, the perimeter band width, and it is blocked on nothing: no fit
feeds it and it feeds no fit. It is four `FrameTile` copies read against four
1:1 paper rungs, so it rides **whichever plate has bed room first** — including
Plate 1 — rather than waiting behind W-F1. Sequencing it here would be an
invented dependency, and an invented dependency is the cheapest way to leave a
bet open for months.
...
the off-grid rosette brick, whose `.bkr` **has since been authored** —
`patterns/Lego/Rosette-Brick.bkr`, bikar `bf6c602`, riding the `footprint outline`
mode that shipped in the same commit, so what it still waits on is LG-F1's rung
and not a model; **LG-P1** the mural seam-registration pair (needs LG-F1's rung; its
...
**Count: 32 <!--count:catalog-entries--> catalog entries = 32
...
against the 10 <!--count:cal-bets-mc--> of 22 <!--count:cal-bets--> and 15
<!--count:cal-mc-records--> of 21 <!--count:cal-records--> that `bets.md` prints
...
- **Counts reconcile — and are copied from the thing that prints them, not
  re-derived here.** 22 <!--count:cal-bets--> bets = 10 <!--count:cal-bets-mc--> on the card + 10 <!--count:cal-bets-design--> on design coupons
  ...
  `CAL-FRM-01` W-P1, `CAL-GRP-01` LG-D1) + 2 <!--count:cal-bets-no-coupon--> with no coupon (`CAL-STR-01`, `CAL-EQV-01`). The card's 10 are the
  ...
  21 <!--count:cal-records--> records = 15 <!--count:cal-mc-records--> on the
  card + 6 <!--count:cal-design-records--> on design coupons — four design-coupon bets (the mural pair and the
  brick-anchor pair) have a coupon but no bikar record yet, which is why the
  record count does not track the bet count. 32 <!--count:catalog-entries--> print-gated items = 32 <!--count:catalog-entries--> catalog
  ...
  On 2026-08-03 every number in this bullet and in §2's table was **wrong** —
  stale by three merges, while the bullet's own job was to catch exactly that.
  A hand tally is a claim with no owner: nothing re-runs it. So each number now
  names what prints it — `make validate-catalog` for the entry count,
  [`bets.md`](../.claude/skills/calibrate/bets.md) (generated by bikar's
  `registry:calibration`, do not hand-edit) for the bet and record counts — and
  ...
  table is correct and whose §8 bullet, restating the same figure, was not
