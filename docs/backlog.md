...
| `tile-wall` §10 Q2 — `checker` with two tile types: enforce shared border geometry by construction, or validate per pair? | a design decision | [`tile-wall-design.md`](tile-wall-design.md) §10 |
| `tile-wall` §10 Q3 — do cropped edge tiles keep relief clipped mid-motif, or does the border band thicken? Leaning: offer both | a design decision | [`tile-wall-design.md`](tile-wall-design.md) §10 |
| `print-validation` §"Open" — should F3 (supports required) ever be an *error* for gallery presets? | a policy decision | [`print-validation-design.md`](print-validation-design.md) |
