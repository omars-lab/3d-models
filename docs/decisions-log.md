...
D-068 chose `border <pattern> width <mm>` as a clause of the `coaster` block and
sequenced it after the interlock (D-069) and the Coaster Lab (D-067), both shipped.
What it left open is how a second pattern is laid into a band that follows an
outline the importer chooses per construction (D-066: round, square, diamond,
hexagon, octagon). The author of a border motif cannot know which outline the field
will get; the kernel is the only party that does.

### Options on the table

- **(a) Plane-clipped** — draw the border pattern in the coaster's plane and mask
  its relief to the annulus. A ring motif is cut by every flat of a polygon, a
  square motif by every arc of a disc; the author draws the band's shape as well as
  its art.
- **(b) Strip-mapped per run** — the border pattern is a motif cell (its bounding
  box, `+y` outward); the kernel lays `⌊U/ℓ⌋` copies along each flat of a polygon
  outline (affine; the corners stay plain ground) and `round(C/ℓ)` copies around a
  round outline (polar; each copy stretched by at most `1/(2N)` so the ring closes),
  into the strap-safe band `[w/2 + t, W − w/2]`.
- **(c) Corner motifs** — (b) plus a `corner <pattern>` cell mitred into each
  vertex: a second cell, a mitre mapping and a second enclosure check.
- **(d) Stock band styles** — `border chevron width 8`, the art generated by the
...
`border IDENT width ConstExpr`, optional, once, no new reserved word (`border` and
`width` are already words the parser knows; the coaster body dispatches on text).
The coaster's one `relief` and one `strap width` govern both regions;
`reliefAppliesAt` reads the placed band art where `0 < ι(p) < W` and the field art
where `ι(p) ≥ W`, `ι` being the exact nominal-outline inset CV7 already uses. CV7
measures the field against the band's inner edge (`ι − W ≥ w/2` at every strap end).
CV11 is new: every run holds at least one whole motif, and every placed strap end
lies in the strap-safe band, per part, with `N` and the closure stretch in the
message. `rim` is refused with `border` (one clause owns the annulus) and so is
`outline pattern` (the band is relief on ground; a minimal coaster has none —
lifting that is a classification change on the same placement, after CS-5 prints).
`interlock` and `trivet` compose. The field records a per-cell `band` bit for #37.
The importer's `--border` emits a stock chevron motif pattern and knobs
`border = 8` (a choice, no bet — a seated strap on a slab is CAL-CST-01's case, the
K10 sentence written in §7) and `margin`; `unit = (size − 2·(border + margin)) / K`;
the band does not scale with `size`, so the mini passes `--param border=4`.

### What would reverse it

A printed CS-5 whose plain polygon corners read as a gap — that sends the form to
(c). A band that the slicer's per-region filament needs to be a *separate body*
rather than a region of one — that is #37's question and would not change the
placement, only the export. A real border that is not a repeat (an inscription, a
single asymmetric wreath) — the cell model cannot express it and would need the
plane-clipped path (a) as a second mode.
