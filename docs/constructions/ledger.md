...
**5 <!--count:constructions-migrated--> migrated** so far: a row counts as
...
octagon frame the least-area default fit, no pin), and `rDuxHF3xMOc` with
bikar PR #227 (the importer now lowers a `Reflect`/`Rotate` of a brace-group
**list literal** into a conjugated orbit rather than refusing it). All five now
vendor a standard (90 mm) coaster mesh under `src/Coasters/` and carry a
prototype-catalog entry (`CS-1`, `CS-2`, `CS-6`, `CS-7`, `CS-8`) — P3.3 of the
