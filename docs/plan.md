...
Last updated: 2026-09-02 (6.6.3 shipped). This is the **index of record** for active work across the
...
| 2.2 | Explorer open ledger — widen the roster (6.6.1 🟢 2026-09-01, bikar #134 `85269ac`), ground the explorer doc (6.6.2 🟢 2026-09-01, 3d-models #136), plates as data (6.6.3 🟢 2026-09-02, bikar #141 `571cba2`), interior-tube cap (6.6.4) | 🔵 | none | explorer doc §6.6 |
...
| 2.10 | **Fourth orb on the M4c quantized lattice walk**, built as the measurement for the process: every stop an earlier orb also needed is logged and labelled *detector* (becomes a gate or test in the same PR) or *instruction*; an orb skill is written only if the instruction list is non-empty when the orb ships | ⚪ | none | D-049 §5, maclado doc, skill-evaluation precedent |
| 2.11 | **Flat→sphere wrap morph** for the breakdown page — grounded design doc first (per-vertex travel from the face-lift plane to the sphere by one parameter; unstyled `morph` frames on the same viewBox so the timelapse gate's junction identities hold; what the woven family shows), then one bikar PR and one 3d-models PR | ⚪ | none | D-049 §2, timelapse doc |
| 2.12 | **Decision hub** — bikar's ledger script indexes this repo's `## D-0xx` headings as links, nothing copied; then the cross-repo ledger check blocks on a D-number that resolves in neither repo | ⚪ | none | D-049 §3, D-004 |
| 2.13 | **Studio status page** — three facts read from files, none typed: bikar commit the gallery was built from, use-cases `as_of` pins, last deploy; rendered like the studio index, with a test holding it to the filesystem | ⚪ | none | D-049 §4 |
| 2.14 | **Rosette-N sweep record** — the three parked value-triples plus the working tree as a table beside the pattern in bikar, merged by PR; stashes dropped only after it is on origin | 🔵 | none | D-049 §6 |
...
| 2026-09-02 | **Explorer plates are data, gated at load** (6.6.3) — `PLATES` → `data/plates.json` `{id, studs, mm, brand}`; `loadPlates` refuses a duplicate id, non-integer studs, a nominal plate whose mm ≠ 8 × studs, or a default naming no plate; select populated from the roster; verified in Chrome | bikar #141 `571cba2`; 3d-models #PRNUM |
